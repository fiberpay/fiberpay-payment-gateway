...
function fiberpaygw_log_debug( $message, $context = array() ) {
...
add_action(
	...
	function () {
		...
		if ( ! class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' )) {
			fiberpaygw_log_debug( 'AbstractPaymentMethodType class does not exist' );
			return;
		}
		...
		require_once __DIR__ . '/includes/blocks/class-fiberpay-blocks.php';
		...
		add_action(
			...
			function () {
				...
				if ( ! function_exists( 'register_block_type' )) {
					fiberpaygw_log_debug( 'Block editor not available' );
					return;
				}
				...
				wp_register_script(
					...
					array( 'wp-element', 'wp-components', 'wp-blocks', 'wp-block-editor', 'wc-blocks-registry', 'wc-settings' ),
					'1.0.0',
					true
				);
					...
					if (isset( WC()->payment_gateways ) && WC()->payment_gateways) {
						...
						if (isset( $gateways['fiberpay'] )) {
							...
							$payment_data = array(
								...
								'is_test_env' => 'yes' === $gateway->is_test_env,
			...
			20
		); // Increased priority to ensure WooCommerce is fully loaded
	}
);
...
function fiberpaygw_init_gateway_class() {
	...
	if ( is_admin() ) {
		require_once __DIR__ . '/includes/class-fiberpaygw-admin-notices.php';
	}
	...
	if (class_exists( 'WC_Payment_Gateway' )) {
		...
		fiberpaygw_log_debug( 'WC_Payment_Gateway class does not exist', array( 'source' => 'fiberpay-payment-gateway' ) );
	}
}
